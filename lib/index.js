...
discordBot.on('ready', () => {
    discordBot.channels
        ...
        .forEach((channel) => {
        const channelName = channel.name || channel.recipient.username;
        console.log(channelName);
        discordChannelMap[channelName.replace(/ /g, '-').slice(0,18)] = channel;
    });
});
...
.then((channels) => {
    ...
    slack.setupRTM(({channel, user, text}) => {
        ...
        if(!discordChannel){
            return console.error('No channel found for', 
                channelName, 
                discordName, 
                Object.keys(discordChannelMap));
        } 
